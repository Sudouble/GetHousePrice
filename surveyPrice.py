...
def get_page(url):
    headers = {
        'User-Agent': r'Mozilla/5.0 (Windows NT 6.1; WOW64) AppleWebKit/537.36 (KHTML, like Gecko) '
                      r'Chrome/50.0.2661.102 Safari/537.36 115Browser/6.0.3',
...
# 获取房源信息详情
def get_info_pn_list_index(search_list, index):
    fin_search_list = []
    print('>>>正在抓取%s' % search_list[index][:3])
    search_url = search_list[index][3]
    try:
    ...
    for pn in range(1, max_pn+1):
        ...
        tmp_url_list = copy.deepcopy(search_list[index][:3])
    ...
    for i in range(len(fin_search_list)):
        ...
        for num in range(20):
            ...
            try:
                ...
                price_UnitPrice_InfoSource = price_list[num].get_text()
                # unit_price = re.search('\d+[\u4E00-\u9FA5][\u002f][\u33a1]', price_list[num].get_text()).group(0)
                tag_tmp_list = copy.deepcopy(fin_search_list[i][:3])
                for tag in [title, address, area, layout, floor, price, price_UnitPrice_InfoSource]:
...
def save_excel(fin_info_list, file_name):
    ...
    book = xlsxwriter.Workbook(r'%s.xls' % file_name)  # 默认存储在桌面上
...
if __name__ == '__main__':
    # file_name = raw_input(r'抓取完成，输入文件名保存：')
    file_name = 'ddd'
    ...
    for k in search_dict:
        ...
        for s_k in second_dict:
            ...
            if not third_search_dict:
                third_search_dict = {'none':second_dict[s_k]}
